...
btnLogin.addEventListener("click", async function () {
    ...
    const estudianteEncontrado =estudiantes.filter(est =>
        est.userName === usuarioLogin.user &&
        est.passwordRegister === usuarioLogin.password
    );
    loginEspacio.innerHTML="";
    if(!usuarioLogin.user||!usuarioLogin.password){
        const mensajeLogin=document.createElement("h2");
        mensajeLogin.textContent="Por favor llenar todos los campos";
        mensajeLogin.style.color="red";
        loginEspacio.appendChild(loginEspacio);
        return;
    }
    if(estudianteEncontrado.length>0){
        window.location.href="paginaPrinci"
    }
})
